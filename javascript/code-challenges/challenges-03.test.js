...
const addTwo = (arr) => arr.map(AddedTwo => AddedTwo + 2);
...
const typeNum = (arr) => arr.filter(onlyNum => typeof onlyNum === 'number');
...
const containsAnd = (arr) => arr.filter(AndONLY => AndONLY.includes('and'));
...
const oddValues = (arr) => arr.filter(oddOnly => oddOnly % 2 !== 0);
...
const notInFirstArray = (forbiddenValues, arr) => arr.filter(notInArray => !forbiddenValues.includes(notInArray));
...
const getBaseStatGreaterThan = (arr, minBaseStat) => arr.filter(stats => stats.baseStat > minBaseStat);
