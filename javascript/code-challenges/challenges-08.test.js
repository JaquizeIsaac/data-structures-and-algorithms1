...
const sortStarWarsCharacters = (starWarsArr) => starWarsArr.sort((shortest, tallest) => parseInt(tallest.height) - parseInt(shortest.height));
...
const listFoods = (recipe) => {
  let result = recipe.ingredients.map(ingredient => {
    const firstSpaceIdx = ingredient.indexOf(' ', ingredient.indexOf(' ') + 1);
    return ingredient.slice(firstSpaceIdx + 1);
  });
  return result;
};
