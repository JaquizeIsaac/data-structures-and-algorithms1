...
const addValues = (arr) => arr.reduce((counter, number) => counter + number, 0);
...
const addPurchases = (arr) => arr.reduce((counter, number) => counter + number.purchasePrice, 0);
...
const countNumberOfElements = (arr) => arr.reduce(counter => counter + 1, 0);
...
const returnNames = (arr) => arr.reduce((accumulator, char) => {
  accumulator.push(char.name);
  return accumulator;
}, []);
...
const reversedString = (str) => str.split('').reduce((reversi, character) => character + reversi, '');
